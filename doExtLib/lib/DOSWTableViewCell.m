...
@interface DOSWTableViewCell () <UIScrollViewDelegate,  UIGestureRecognizerDelegate,DOSWUtilityButtonViewDelegate>
...
@property (nonatomic, strong) DOSWUtilityButtonView *leftUtilityButtonsView, *rightUtilityButtonsView;
...
@implementation DOSWTableViewCell {
...
- (void)initializer
{
    ...
    self.cellScrollView = [[DOSWCellScrollView alloc] init];
    ...
    self.longPressGestureRecognizer = [[DOSWLongPressGestureRecognizer alloc] initWithTarget:self action:@selector(scrollViewPressed:)];
...
//    [self.cellScrollView addGestureRecognizer:self.longPressGestureRecognizer];
    ...
    self.leftUtilityButtonsView = [[DOSWUtilityButtonView alloc] initWithUtilityButtons:nil
                                                                           parentCell:self
    ...
    self.rightUtilityButtonsView = [[DOSWUtilityButtonView alloc] initWithUtilityButtons:nil
                                                                            parentCell:self
...
- (void)setLeftUtilityButtons:(NSArray *)leftUtilityButtons
{
    ...
    self.leftUtilityButtonsView.delegate = self;
...
- (void)setRightUtilityButtons:(NSArray *)rightUtilityButtons
{
    ...
    self.rightUtilityButtonsView.delegate = self;
...
#pragma mark - DOSWUtilityButtonViewDelegate 方法
...
{
    DOSWUtilityButtonTapGestureRecognizer *utilityButtonTapGestureRecognizer = (DOSWUtilityButtonTapGestureRecognizer *)sender;
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
    ...
    if (_cellState != kCellStateCenter)
    {
        if ([self.delegate respondsToSelector:@selector(swipeableTableViewCellShouldHideUtilityButtonsOnSwipe:)])
        {
            for (DOSWTableViewCell *cell in [self.containingTableView visibleCells]) {
                if (cell != self && [cell isKindOfClass:[DOSWTableViewCell class]] && [self.delegate swipeableTableViewCellShouldHideUtilityButtonsOnSwipe:cell]) {
...
{
    BOOL _is = ![touch.view isKindOfClass:[self.cellScrollView class]];
    return _is;
}
